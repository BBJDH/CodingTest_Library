...
void Push(Node const & VisitPos)
{
	if (VisitPos.X >= 0 and VisitPos.Y >= 0 and 
		...
		Visits[VisitPos.Y][VisitPos.X] == false )
	{
...
void VisitSides(Node const& CurrentPos, bool IsRB)
{
	for (int i = 0; i < 4; i++)
	{
...
void FindGroup(Node const& StartPos)
{
	if (Visits[StartPos.Y][StartPos.X] == true)
		return;
	CurrentColor = Paints[StartPos.Y][StartPos.X];
	Push(StartPos);
	while (!BFS.empty())
	{
...
void Solution()
{
	...
	for (auto const& Elem : StartPositions_RB)
		FindGroup(Elem);
	cout << GroupCount;
}
...
int main()
{
	...
	for (int i = 0; i < PaintSize; i++)
	{
		for (int j = 0; j < PaintSize; j++)
		{
			...
			if (PrevColor != InputColor)
			{
				PrevColor = InputColor;
				StartPositions.push_back({ j,i,{InputColor} });
				if(InputColor == 'R' or InputColor == 'G')
					StartPositions_RB.push_back({ j,i,{'R','G'}});
				else
					StartPositions_RB.push_back({ j,i,{'B',} });
			}
		}
	}
	Solution();
